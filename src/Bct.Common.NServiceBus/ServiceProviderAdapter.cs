// <copyright file="ServiceProviderAdapter.cs" company="TerumoBCT">
...
internal class ServiceProviderAdapter : IBuilder
{
   ...
   {
      return serviceProvider.GetService(typeToBuild) ?? throw new Exception(
                $"Unable to build {typeToBuild.FullName}. Ensure the type has been registered correctly with the container.");
   }
   ...
   private class ChildScopeAdapter : IBuilder
   {
      ...
      public object Build(Type typeToBuild)
      {
         return serviceScope.ServiceProvider.GetService(typeToBuild) ?? throw new Exception(
                   $"Unable to build {typeToBuild.FullName}. Ensure the type has been registered correctly with the container.");
      }
